...
# replace this email id with the one for which you have created client_secrets.json (i.e., your college domain id)
email_sender = 'username@domain.com'
email_recipient = 'username@domain.com'  # you can replace this with your personal email id
...
def violation_alert_generator(im0, subject='People Count Limit Exceeded', message_text='There are more than 3 number '
                                                                                       'of persons detected'):
    """
        ...
        try:
            ...
            while True:
                ...
                for i, det in enumerate(pred):
                    ...
                    if len(det):
                        # Rescale boxes from img_size (predicted image) to im0 (actual image) size and round the
                        # coordinates
                    ...
                    yield cv2.putText(img0, f"Persons Detected: {total_detections}", (120, 150),
                                      cv2.FONT_HERSHEY_SIMPLEX, 2, (255, 0, 0), 5, cv2.LINE_AA), total_detections
